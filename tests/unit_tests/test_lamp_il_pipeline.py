...
def test_dp_policy_can_execute_newest_plan_without_temporal_ensemble(monkeypatch):
    ...
    monkeypatch.setattr(
        policy,
        "encode_observation",
        lambda _: LampObservationFeatures(condition=torch.zeros(1, 256)),
    )
    monkeypatch.setattr(
        policy, "sample_base_plan", lambda *args, **kwargs: LampPlan(core, physical)
    )
...
def test_vq_task_configs_defer_epoch_steps_to_runtime(task, steps_per_epoch):
    ...
    assert cfg.runner.save_every_epochs == 10
