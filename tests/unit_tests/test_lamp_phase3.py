...
def test_lamp_online_sac_configs_use_exec8_v4(task: str) -> None:
    ...
    assert cfg.runner.max_steps == 12000
    assert cfg.runner.logger.step_axis == "env_step"
    assert cfg.algorithm.replay_buffer.auto_save is True
    assert cfg.algorithm.replay_buffer.cache_size == 1000
    assert cfg.algorithm.replay_buffer.sample_window_size == 200000
...
@pytest.mark.parametrize(
    "config_name",
    (
        "dexjoco_lamp_residual_v4_online_cvae_hammer_nail_pd_cadence",
        "dexjoco_lamp_residual_v4_online_decoder_only_hammer_nail_pd_cadence",
        "dexjoco_lamp_residual_v4_online_decoder_only_water_plant_pd_cadence",
        "dexjoco_lamp_residual_v4_online_pca_hammer_nail_pd_cadence",
        "dexjoco_lamp_residual_v4_online_pca_water_plant_pd_cadence",
        "dexjoco_lamp_residual_v4_online_mlp_water_plant_a07_seed42",
    ),
)
