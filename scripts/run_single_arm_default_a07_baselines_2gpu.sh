#!/usr/bin/env bash
# Five-task A07 transfer: PCA, raw MLP, and VQ on an independent 2-GPU host.
set -euo pipefail
...
OUTPUT_ROOT="${OUTPUT_ROOT:-${REPO_ROOT}/outputs/lamp_single_arm_default_a07_2gpu/${HOST_TAG}}"
...
TASKS=(click_mouse pinch_tongs hammer_nail fold_glasses pick_bucket)
MODES=(pca mlp vq)
...
run_prior() {
  ...
  case "${prior}" in
    pca_dim2) name="${task}_prior_pca_z2" ;;
    vq) name="${task}_prior_vq" ;;
...
prior_artifact() {
  local task="$1" mode="$2"
  case "${mode}" in
    pca) printf '%s' "${OUTPUT_ROOT}/train/${task}_prior_pca_z2/artifact" ;;
    vq) printf '%s' "${OUTPUT_ROOT}/train/${task}_prior_vq/artifact" ;;
    mlp) printf '%s' '-' ;;
  esac
}

run_dp() {
  local gpu="$1" task="$2" mode="$3" config name run_dir artifact prior contract checkpoint prior_sha latent
  config="dexjoco_lamp_dp_il_${mode}_${task}"
  name="${task}_dp_a07_${mode}_seed${TRAIN_SEED}"
  run_dir="${OUTPUT_ROOT}/train/${name}"
  artifact="${run_dir}/artifact"
  prior="$(prior_artifact "${task}" "${mode}")"
  latent='-'; prior_sha=none
  local -a prior_args=()
  if [[ "${prior}" != '-' ]]; then
    [[ "${mode}" == pca ]] && latent=2 || latent=1
    if [[ "${DRY_RUN}" == 1 ]]; then
      prior_sha=dry_run
    else
    ...
    prior_args+=("actor.model.hand_prior.latent_dim=${latent}" "actor.model.hand_prior.artifact_path=${prior}")
  fi
  contract="dp|${config}|task=${task}|mode=${mode}|prior=${prior}|prior_sha=${prior_sha}|z=${latent}|lr=1e-4|bb=0.03|wd=1e-3|warm=1000|hand=0.0|seed=${TRAIN_SEED}|data=${DATASET_ROOT}|cache=${CACHE_ROOT}"
  check_contract_or_write "${run_dir}" "${contract}" || return 1
  ...
  if ! "${PYTHON_BIN}" examples/embodiment/train_lamp_il.py \
      ...
      "data.dataset_root=${DATASET_ROOT}" "data.cache_root=${CACHE_ROOT}" \
      "algorithm.bc_loss.hand=0.0" "actor.seed=${TRAIN_SEED}" \
      "actor.optim.lr=1e-4" "actor.optim.backbone_lr_ratio=0.03" \
      "actor.optim.weight_decay=1e-3" "actor.optim.warmup_steps=1000" \
      "${prior_args[@]}" \
      "runner.logger.log_path=${OUTPUT_ROOT}/train" "runner.logger.experiment_name=${name}" \
      "${resume[@]}" >"${OUTPUT_ROOT}/logs/${name}.log" 2>&1; then
...
with initialize_config_dir(version_base=None, config_dir=str(Path('examples/embodiment/config').resolve())):
    for task in tasks:
        compose(config_name=f'dexjoco_lamp_prior_pca_dim2_{task}', overrides=['algorithm.bc_loss.hand=0.0', 'actor.seed=42'])
        compose(config_name=f'dexjoco_lamp_prior_vq_{task}', overrides=['algorithm.bc_loss.hand=0.0', 'actor.seed=42'])
        for mode in ('pca', 'mlp', 'vq'):
            overrides=['algorithm.bc_loss.hand=0.0', 'actor.seed=42', 'actor.optim.lr=1e-4', 'actor.optim.backbone_lr_ratio=0.03', 'actor.optim.weight_decay=1e-3', 'actor.optim.warmup_steps=1000']
            if mode == 'pca': overrides.append('actor.model.hand_prior.latent_dim=2')
            cfg = compose(config_name=f'dexjoco_lamp_dp_il_{mode}_{task}', overrides=overrides)
            assert float(cfg.algorithm.bc_loss.hand) == 0.0
            assert (float(cfg.actor.optim.lr), float(cfg.actor.optim.backbone_lr_ratio), float(cfg.actor.optim.weight_decay), int(cfg.actor.optim.warmup_steps)) == (1e-4, 0.03, 1e-3, 1000)
...
log "launcher started output=${OUTPUT_ROOT} cache=${CACHE_ROOT}"
preflight_compose
log "preflight passed: tasks=5 priors=10 policies=15 evals=15"
[[ "${PREFLIGHT_ONLY}" == 1 ]] && exit 0

PRIOR_JOBS=(); DP_JOBS=(); EVAL_JOBS=()
for task in "${TASKS[@]}"; do
  PRIOR_JOBS+=("${task}|pca_dim2" "${task}|vq")
...
for task in "${TASKS[@]}"; do for mode in "${MODES[@]}"; do
  eid="${task}_dp_a07_${mode}_seed${TRAIN_SEED}_k8_ddim16_env0-49"
