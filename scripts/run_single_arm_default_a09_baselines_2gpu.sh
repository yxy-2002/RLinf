#!/usr/bin/env bash
# Six-task A09 transfer: PCA, raw MLP, and VQ on an independent 2-GPU host.
set -euo pipefail
...
run_dp() {
  ...
  contract="dp|${config}|task=${task}|mode=${mode}|prior=${prior}|prior_sha=${prior_sha}|z=${latent}|lr=1e-4|bb=0.3|wd=1e-4|warm=500|hand=0.0|seed=${TRAIN_SEED}|data=${DATASET_ROOT}|cache=${CACHE_ROOT}"
  check_contract_or_write "${run_dir}" "${contract}" || return 1
...
with initialize_config_dir(version_base=None, config_dir=str(Path('examples/embodiment/config').resolve())):
    for task in tasks:
        ...
        for mode in ('pca', 'mlp', 'vq'):
            overrides=['algorithm.bc_loss.hand=0.0', 'actor.seed=42']
            if mode == 'pca': overrides.append('actor.model.hand_prior.latent_dim=2')
            cfg = compose(config_name=f'dexjoco_lamp_dp_il_{mode}_{task}_a09', overrides=overrides)
