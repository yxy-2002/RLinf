...
GROUP=3
SESSION="lamp-lstm-il-remaining-$GROUP"
OUTPUT="$REPO_ROOT/outputs/lamp_lstm_il/remaining_three/group_$GROUP"
ARGS=()
FOREGROUND=0
PREPARE=0
for arg in "$@"; do
...
if (( FOREGROUND || PREPARE )); then
    exec "$REPO_ROOT/.venv/bin/python" -u -m scripts.lamp_lstm_il_remaining --group "$GROUP" "${ARGS[@]}"
fi
...
"$REPO_ROOT/.venv/bin/python" -m scripts.lamp_lstm_il_remaining --group "$GROUP" --prepare
"$REPO_ROOT/.venv/bin/ray" status >/dev/null
mkdir -p "$OUTPUT"
printf -v launch '%q ' env WANDB_MODE=offline OMP_NUM_THREADS=2 MKL_NUM_THREADS=2 OPENBLAS_NUM_THREADS=1 PYTHONPATH="$PYTHONPATH" EMBODIED_PATH="$EMBODIED_PATH" "$REPO_ROOT/.venv/bin/python" -u -m scripts.lamp_lstm_il_remaining --group "$GROUP" "${ARGS[@]}"
