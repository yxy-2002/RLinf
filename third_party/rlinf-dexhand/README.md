...
反馈按配置的 `motor_ids` 顺序组织，不依赖串口响应到达顺序。
缺失电机的位置保留上次值；未知 ID、空响应和非有限位置被忽略。
同一电机重复回复时，使用最后一个有效位置。部分有效反馈会更新对应电机，
缺失电机的速度、电流置零；整轮无有效反馈时保留缓存。
串口短读保留未完整的尾帧，并在后续读取时拼接；反馈通过帧头、方向、长度、
指令和求和校验后才进入状态更新。损坏帧逐字节重新同步，不清空每轮串口缓冲。
默认每条电机命令后等待 1ms，可通过 `command_interval_s` 配置；RLinf 对应
`env.eval.override_cfg.end_effector_config.command_interval_s`。串口独占打开，
写超时为 0.1 秒，短写入明确报错。

`get_detailed_state()` 的 `missing_motor_ids` 列出最近一轮缺失的电机；
`feedback_valid` 仅在最近一轮全部配置电机均有有效位置、且反馈年龄不超过
0.5 秒时为真。`feedback_timestamp` 表示最近一次至少收到一个有效电机反馈的时间，
不能单独用它判断整只手的反馈完整性。

此变更仅对齐反馈 ID，不改变手套遥操作的按下时基准映射。

`feedback_age_s_by_motor` 给出各电机有效反馈的主机处理年龄（秒），未收到时
为 `None`；它不是设备采样时间。
